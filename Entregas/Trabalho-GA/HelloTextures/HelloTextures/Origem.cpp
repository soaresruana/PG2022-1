...
int main()
{
	...
	texIDBackground = loadTexture("../textures/parede.jpg");
	...
	//glViewport(0, height / 2, width / 2, height / 2); //ESQUERDA SUPERIOR
	...
	// Poligono Preenchido - GL_TRIANGLES
	// Observe que como  usamos EBO, agora usamos a glDrawElements!
	//glViewport(width / 2, height / 2, width / 2, height / 2); //DIREITA SUPERIOR
	//glBindVertexArray(VAO);
	//glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
	//glBindVertexArray(0); //unbind - desconecta
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_W && action == GLFW_PRESS) {
		glfwSetWindowTitle(window, "MARIO");
		if (y > -2.8f)
			y -= (float)glfwGetTime() * 0.1f;
		texIDface = loadTexture("../textures/mario2.png");


	}

	if (key == GLFW_KEY_A && action == GLFW_PRESS) {
		glfwSetWindowTitle(window, "FLOR");
		if (x < 2.8f)
			x += (float)glfwGetTime() * 0.1f;
		texIDface = loadTexture("../textures/flor.png");
	...
	if (key == GLFW_KEY_S && action == GLFW_PRESS) {
			glfwSetWindowTitle(window, "MONSTRO");
				if (y < 2.8f)
					y += (float)glfwGetTime() * 0.1f;

			texIDface = loadTexture("../textures/monstro.png");
		
	}

	if (key == GLFW_KEY_D && action == GLFW_PRESS) {
		glfwSetWindowTitle(window, "YOSHI");
		if (x > -2.8f)
			x -= (float)glfwGetTime() * 0.1f;
		texIDface = loadTexture("../textures/yoshi.png");
